...
namespace WingCommand
{
    ...
    /// its angle about the velocity axis is the bank. One law for lateral and vertical manoeuvres.
    /// The lift vector never points below the horizon of the velocity frame: a descent demand beyond
    /// −1 g saturates at a zero-g push-over with the wings where they are, never an inverted pull.</summary>
    internal static class AccelMapping
    {
        ...
        public static AttitudeCommand Map(in GuidanceCommand cmd, Vec3 velocity, float previousBankDeg)
        {
            ...
            if (right.SqrLength > 1e-6f)
            {
                ...
                float lateral = Vec3.Dot(lift, right), vertical = Math.Max(0f, Vec3.Dot(lift, up));
                result.Nz = (float)Math.Sqrt(lateral * lateral + vertical * vertical) / Scalar.G;
                if (result.Nz >= MinLiftForBank)
                    result.BankDeg = (float)Math.Atan2(lateral, vertical) * Scalar.Rad2Deg;
            }
