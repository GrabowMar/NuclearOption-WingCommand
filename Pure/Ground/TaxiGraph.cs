...
namespace WingCommand
{
    ...
    /// each service point (where hangar-less launches stand); a node on each runway exit (the game marks them on the
    /// centreline: the connector to the nearest road is the way off the runway); per
    ...
    internal sealed class TaxiGraph
    {
        ...
        private readonly List<int> runwayExits = new List<int>();
        ...
        /// <summary>Every runway exit node (the way off a runway after a landing).</summary>
        public IReadOnlyList<int> RunwayExits => runwayExits;
        ...
        public static TaxiGraph Build(AirbaseSample field, float mergeRadius = 10f)
        {
            ...
            if (roads)
                foreach (RunwaySample runway in field.Runways)
                    foreach (Pose exit in runway.Exits)
                    {
                        int node = g.Synthetic(exit.Pos, NodeKind.RunwayExit);
                        if (!g.runwayExits.Contains(node)) g.runwayExits.Add(node);
                    }
