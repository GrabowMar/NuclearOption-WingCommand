...
namespace WingCommand
{
    ...
    /// over a point (<see cref="FlyHover"/>: within <see cref="HoverFace"/> it slides straight to the point, as a
    /// helicopter moves without turning, while it yaws to the task's heading, and climbs or descends at up to
    /// <see cref="HoverClimbRate"/>). The lateral law is a vector field: the course is the path's direction turned
    ...
    internal sealed class TaskLead
    {
        ...
        public TaskLead(Vec3 position, Vec3 velocity, bool canHover)
        {
            Position = position;
            Speed = velocity.Length;
            HeadingDeg = velocity.Horizontal.SqrLength > 1f ? Vec3.HeadingDeg(velocity) : 0f;
            CanHover = canHover;
        ...
        public void FlyHover(Vec3 point, float headingDeg, float altitude, float cruise, float dt)
        {
            ...
            if (d > (sliding ? 4f : 1f) * HoverFace)
            {
                Steer(Vec3.HeadingDeg(to), 0f, altitude, speed, dt);
                return;
            }
            sliding = true;
            if (d > 0.01f) track = to * (1f / d);
            Speed = Slew.Step(Speed, speed, SpeedRate, dt);
