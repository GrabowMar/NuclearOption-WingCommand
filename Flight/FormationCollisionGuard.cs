    ...
    internal static class FormationCollisionGuard
    {
            ...
            float spacing, out Vector3 escape, out Aircraft threat, out float miss)
        {
            ...
            for (int i = -1; i < count; i++)
            {
                ...
                float selfSize = self.definition == null ? 0f :
                    Mathf.Max(self.definition.length, self.definition.width, self.definition.height);
                float otherSize = other.definition == null ? 0f :
