...
- **Module Facades**: Code outside `Personnel/`/`Combat/`/`Economy/` must call them through `PersonnelFacade`/`CombatFacade`/`EconomyFacade`, never the internal classes (`WingPilotRoster`, `WingWeapons`, `WingShop`, `WingLoadoutCatalog`, etc.) directly. Each facade is a static class with one nested class per internal subsystem — `PersonnelFacade`: `Roster`, `Recruitment`, `Takeover`, `Recovery`, `SearchAndRescue`, `CustomPilots`, `Portraits`, `KillCredit`, `Departure`, `DepartureChatter`; `CombatFacade`: `Weapons`, `Roe`, `Countermeasures`, `Tactical`; `EconomyFacade`: `Shop`, `LoadoutCatalog`, `LoadoutTemplates`, `LoadoutBook`, `SupplyReserve`, `LaunchFields`, `Airfield`, `DepartureLane`, `ShopDelivery`, `HangarStock` — each member forwarding 1:1 to the internal implementation with no renaming and no behavior change. A module's own nested data types (e.g. `WingShop.Offer`, `WingLoadoutCatalog.StoreOption`) stay directly referenced by name in callers, same as `WingPilot` — the facade wraps static operations, not caller-held data types. Files inside a module may call each other directly.
- **Avionics & Shared UI**:
